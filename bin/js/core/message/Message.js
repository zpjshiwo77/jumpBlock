class Message {
     ...
     * @param listener      侦听函数
     */
    static on(type, listenerObj, listener) {
        let arr = this.dic[type];
        if (arr == null) {
            arr = new Array();
            this.dic[type] = arr;
        }
     ...
     * @param listener      侦听函数
     */
    static off(type, listenerObj, listener) {
        let arr = this.dic[type];
        if (arr == null) {
            return;
        }
     ...
     *
     */
    static event(type, ...param) {
        if (this.dic[type] == null) {
            return;
        }
        let vo = ObjectPool.getItem("MessageVo");
        if (null == vo) {
            vo = new MessageVo();
        }
        vo.type = type;
     ...
     * @param msgVo
     */
    static dealMsg(msgVo) {
        let listeners = this.dic[msgVo.type];
        let i = 0;
